...
class InstagramApi
{
	...
	{
		if ( $type == 'image' && ! self::isAppMethod( $accountInfo ) )
		{
		...
		else if ( $type == 'image' && self::isAppMethod( $accountInfo ) )
		{
			$photo = $images[ 0 ];
		}
		...
		if ( $type == 'image' )
		{
			try
			{
				return $apiInstance->uploadPhoto( $accountInfo[ 'id' ], $photo, $message, $link );
			}
			catch ( Exception $e )
			{
				return [
	...
	public static function sendStory ( $accountInfo, $type, $message, $link, $images, $video )
	{
		if ( $type == 'image' )
		{
            ...
            {
                $photo = self::imageForStory( $images[ 0 ], $message, $link );
            }
		}
		else
		{
	 ...
	 * @return array
	 */
	private static function imageForStory ( $photo_path, $title, $link = '' )
	{
