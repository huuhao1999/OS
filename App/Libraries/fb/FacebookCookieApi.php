...
class FacebookCookieApi
{
	...
	public function getMyPages ()
	{
		...
		try
		{
			$result = (string) $this->client->request( 'GET', 'https://www.facebook.com/pages/?category=your_pages&ref=bookmarks', [
                'headers'         => [
        ...
        preg_match( '/\"profile_switcher_eligible_profiles\"\:\{\"nodes\"\:(\[.*?\])/', $result, $pages1 );
        preg_match( '/\"admined_pages\"\:\{\"nodes\"\:(\[.*?\])/', $result, $pages2 );

        $pages1 = empty( $pages1[ 1 ] ) ? [] : json_decode( $pages1[ 1 ], TRUE );
        $pages2 = empty( $pages2[ 1 ] ) ? [] : json_decode( $pages2[ 1 ], TRUE );
        ...
        foreach ( $pages1 as $page )
        {
            $myPagesArr[] = [
                'id'    => $page[ 'id' ],
                'name'  => $page[ 'name' ],
                'cover' => 'https://graph.facebook.com/' . $page[ 'id' ] . '/picture'
            ];
        }
