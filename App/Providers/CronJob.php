...
class CronJob
{
	...
	public static function init ()
	{
        ...
        DB::DB()->query( DB::DB()->prepare( 'UPDATE ' . DB::table( 'feeds' ) . ' SET `is_sended`=%d WHERE `send_time`>%s AND `send_time`<%s AND `is_sended`=%d', [
            0,
            Date::dateTimeSQL( '-2 hours' ),
            Date::dateTimeSQL( '-5 minutes' ),
            2
        ] ) );
